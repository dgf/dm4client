...
(function() {
  var DEL, GET, POST, PUT, assocDefChildTypes, associationEndpoint, associationInfo, async, childTypeUri, clarifyParents, createChildTypes, createOnComplete, dataTypeUri, defaultIcon, detachAsscociation, detachComposite, detachDataType, detachTopic, detachType, fetchComposite, http, iconUri, rest, topicEndpoint, topicInfo, topicsByType, typeInfo, typesEndpoint, url, util, _,
  ...
  createChildTypes = function(assoc_defs) {
    var assoc_def, _i, _len, _ref, _results;
    _results = [];
    for (_i = 0, _len = assoc_defs.length; _i < _len; _i++) {
      assoc_def = assoc_defs[_i];
      if (_ref = assoc_def.type_uri, __indexOf.call(assocDefChildTypes, _ref) >= 0) {
        if (assoc_def.role_1.role_type_uri === childTypeUri) {
          _results.push(assoc_def.role_1.topic_uri);
        } else {
          _results.push(assoc_def.role_2.topic_uri);
        }
      } else {
  ...
  detachType = function(type) {
    var vc, _i, _len, _ref, _ref1, _ref2, _ref3;
    ...
    return {
      ...
      icon: (_ref3 = type.icon) != null ? _ref3 : defaultIcon
    };
  };
